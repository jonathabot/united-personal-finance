...
type ScenarioData = { month: string; category: string; reductionPercentage: number; savingsCents: number; currentCategoryCents?: number; currentProjectedBalanceCents?: number; rows: { scenario: string; categoryCents: number; balanceCents: number }[] };
...
export function ScenarioComparison({ data }: { data: ScenarioData }) {
  return <section className="uiCard chartCard scenarioCard" aria-label={`Comparação de cenário para ${data.category}`}>
    ...
    <dl className="metricGrid scenarioPremises">
      <div><dt>Mês simulado</dt><dd>{data.month}</dd></div>
      <div><dt>Gasto-base</dt><dd>{formatCurrency(data.currentCategoryCents ?? 0)}</dd></div>
      <div><dt>Saldo-base</dt><dd>{formatCurrency(data.currentProjectedBalanceCents ?? 0)}</dd></div>
    </dl>
    <p className="cardNote">Premissa: somente a categoria selecionada é reduzida; receitas e demais compromissos permanecem iguais. A simulação não altera dados.</p>
  </section>;
}
