...
export function CategoryBreakdown({ data }: { data: CategoryVisualData[] }) {
  ...
  const total = data.reduce((sum, item) => sum + item.currentCents, 0);
  return <section className="uiCard chartCard" aria-label="Distribuição dos gastos por categoria">
    <div className="visualHeader"><div><span className="eyebrow">Gasto líquido no mês</span><h3>Gastos por categoria</h3></div><strong>{formatCurrency(total)}</strong></div>
