...
function categoryTotals(transactions: readonly FinanceTransaction[], month: MonthKey) {
  ...
  for (const transaction of transactions) {
    ...
    if (signed !== 0) totals.set(transaction.category, (totals.get(transaction.category) ?? 0) + signed);
  }
  return totals;
}
...
export function analyzeCategorySpending(
  ...
  return [...categories].map((category) => {
    ...
    const baselineTotal = baseline.reduce((total, values) => total + (values.get(category) ?? 0), 0);
